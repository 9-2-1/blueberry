...
export function calculateSpeed(
  ...
  let 完成总数 = 0;
  let 用时总量 = 0;
  const 累积时长 = 速度累积时长 * 60 * 60 * 1000; // 转换为毫秒
  ...
  for (let i = 任务进度记录.length - 1; i > 0; i--) {
    ...
    const 有效用时 = 记录用时 * 24 * 60 * 60 * 1000; // 转换为毫秒

    if (用时总量 + 记录用时 >= 累积时长) {
      const 剩余时间 = 累积时长 - 用时总量;
      const 比例 = 剩余时间 / 有效用时;
      完成总数 += 完成数量 * 比例;
      用时总量 = 累积时长;
      break;
...
export function calculateDailyTime(
  ...
  for (let i = 任务进度记录.length - 1; i > 0; i--) {
    ...
    const 记录用时 = 当前记录.用时; // 单位为天
    const 记录开始 = 记录时间 - 记录用时 * 24 * 60 * 60 * 1000;

    // 计算记录的时间范围
    const 记录开始时间 = Math.max(记录开始, 现在 - 时间范围);
    const 记录结束时间 = Math.min(记录时间, 现在);
    const 有效时间比例 = (记录结束时间 - 记录开始时间) / (记录时间 - 记录开始);
